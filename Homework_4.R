...
plot(deg_dis, main="Degree Distribution #AlevelResults Network", ylim=c(0,400))

# plotting the network
# defining labels according degrees
node_label <- V(networkasigraph)$node_label <- unname(ifelse(degree(networkasigraph)[V(networkasigraph)] > 7, names(V(networkasigraph)), "")) 
...
in_degree1 <- degree(networkasigraph, mode="in") # all, in, out
#out_degree1 <- degree(networkasigraph, mode="out") # all, in, out
#in_degree1
#out_degree1
#mean(in_degree1)
#mean(out_degree1)
in_degree1
V(networkasigraph)$size <- in_degree1*1.41

V(networkasigraph)[V(networkasigraph)$Profile == "Politician"]$color <- "red"
...
V(networkasigraph)[V(networkasigraph)$Profile == "Union Leader"]$color <- "pink"
V(networkasigraph)[V(networkasigraph)$Profile == "Other"]$color <- "green"


plot(networkasigraph, layout=layout_with_dh,
     edge.arrow.size=.19,
